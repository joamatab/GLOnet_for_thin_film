...
def plot_loss_history(loss_history, params):
    ...
    if len(loss_history) == 4:
        effs_90th_history, effs_mean_history, diversity_history, binarization_history = loss_history
        iterations = [i*params.plot_iter for i in range(len(effs_mean_history))]
        plt.figure()
        plt.plot(iterations, effs_mean_history)
        plt.plot(iterations, effs_90th_history)
        plt.xlabel('iteration')
        #plt.legend(('Average efficiency', '90th percentile'))
        ...
        io.savemat(history_path, mdict={'effs_90th_history':np.asarray(effs_90th_history),
                                        'effs_mean_history':np.asarray(effs_mean_history), 
                                        'diversity_history':np.asarray(diversity_history),
                                        'binarization_history' :np.asarray(binarization_history)}) 
